    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (res.success) {
                setContent('');
                setShowInput(false);
                // Comment sạch được thêm qua socket realtime
            }
        } catch (err: any) {
    ...
    return (
        <div className="mt-3 border-t border-slate-100 dark:border-[#334155]/50 pt-2">
            ...
            {comments.map(comment => (
                <div key={comment._id} className="flex items-start gap-2 py-1.5 group text-[13px] text-slate-600 dark:text-[#94a3b8] border-b border-slate-50 dark:border-[#334155]/30 last:border-0">
                    <img
                    ...
                    {user?._id === comment.userId._id && (
                        <button
                            ...
                            className="opacity-0 group-hover:opacity-100 p-1 text-slate-300 hover:text-red-400 transition-all rounded cursor-pointer flex-shrink-0"
                        >
                            <Trash2 size={11} />
                        </button>
                    )}
                    ...
                    {!showInput ? (
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            ...
                            <button
                                ...
                                className="p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                            >
