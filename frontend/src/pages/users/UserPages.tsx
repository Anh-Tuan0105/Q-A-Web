      ...
      <div className="flex flex-1 max-w-[1400px] w-full mx-auto pt-6">
        ...
        <main className="flex-1 ml-8 pr-8 pb-12 w-full max-w-4xl">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-2xl font-bold text-slate-800">Trang chủ Q&A (Public Route)</h1>
            {user && <LogoutButton />}
          </div>
          <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
            {user ? (
              <p className="text-slate-600 mb-4">Chào mừng <span className="font-semibold text-slate-900">{user.displayName}</span> quay trở lại!</p>
            ) : (
              <p className="text-slate-600 mb-4">Chào mừng khách truy cập! Nơi chia sẻ kiến thức cộng đồng.</p>
            )}
            <button
